...
struct Traits<float8_ieee_p<p>> : public TraitsBase<float8_ieee_p<p>> {
  ...
  static constexpr int kExponentBias = 1 << (Base::kExponentBits - 1);
};
...
template <int p>
EIGEN_DEVICE_FUNC inline bool isinf_impl(
    const ml_dtypes::float8_ieee_p<p>& x) {
  return ml_dtypes::float8_internal::isinf(x);
}
...
template <int p>
EIGEN_DEVICE_FUNC inline bool isnan_impl(const ml_dtypes::float8_ieee_p<p>& x) {
  return ml_dtypes::float8_internal::isnan(x);
}
...
template <int p>
EIGEN_DEVICE_FUNC inline bool isfinite_impl(const ml_dtypes::float8_ieee_p<p>& x) {
  return ml_dtypes::float8_internal::isfinite(x);
}
